      ...
      $.ajax({
        ...
        success: function (data){
          ...
          $.ajax({
            type: "POST",
            url: "./consultmes",
            data: { nhotel:id, _token : _token },
            success: function (data){
              if (data != '0') {
                $('#calendar_fecha').val('');
                $('#calendar_fecha').attr('disabled', false);
                $('#calendar_fecha').datepicker('setStartDate', data);
              }
              else {
                $('#calendar_fecha').val('');
                $('#calendar_fecha').attr('disabled', true);
                toastr.error('Sin meses aprobados.!!', 'Mensaje', {timeOut: 1000});
              }
            },
            error: function (data) {
    ...
    if (a0 == true && a1 == true && a2 == true) {
      ...
      $.ajax({
        ...
        success: function (data){
          ...
          if (data == '0') {
            toastr.error('Mes no aprobado.!!', 'Mensaje', {timeOut: 1000});
            $("#contentreport").hide();
            reset_quiz_current();
          }
          if (data == '1') {
            toastr.success('Mes si aprobado.!!', 'Mensaje', {timeOut: 1000});
              ...
              var optiongrafusuarios={
                  ...
                  series : (function (){
                      ...
                      series[0].markPoint = {
                          ...
                          effect:{show:false,scaleSize:7,color:'rgba(250,225,50,0.8)',shadowBlur:10,period:60},
                          data:[{x:'50%',y:'50%'}]
                      };
                      return series;
                  })()
...
$('#generatePdf').on('click', function() {
  ...
  //$('.seleccion').hide();
    ...
    $('#cab_reporte').parent().css("font-size", "12px");
